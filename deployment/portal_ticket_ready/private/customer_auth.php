...
function customer_register(array $data): array {
    ...
    try {
        $stmt = $db->prepare(
            'INSERT INTO customer_accounts
             ...
             VALUES (?, ?, ?, 0, NULL, ?, DATE_ADD(NOW(), INTERVAL 24 HOUR))'
        );
    ...
    return [
        ...
        'message' => portal_email_delivery_enabled()
            ...
            : 'Registrierung erfolgreich. Der E-Mail-Versand ist deaktiviert; bitte kontaktieren Sie MZ Tech zur Aktivierung.',
    ];
}
...
function customer_verify_email(string $token): array {
    ...
    $db->prepare(
        'UPDATE customer_accounts
         SET is_verified = 1, verify_token = NULL, verify_token_hash = NULL, verify_expires = NULL
         WHERE id = ?'
    )
       ->execute([$account['id']]);
    $db->prepare(
