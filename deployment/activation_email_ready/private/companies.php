 ...
 * Aktivierungslink wird anschließend bewusst durch einen Administrator in
 * der Portalverwaltung erzeugt und über den protokollierten Versand gesendet.
 */
function company_contact_create(int $companyId, array $data): array {
    ...
    portal_audit('company_contact_created_pending_activation', 'staff', null, 'company_contacts', $contactId, [
        'company_id' => $companyId,
        'portal_role' => $portalRole,
    ]);
    portal_admin_notification_create('company_contact', $contactId);
    return [
        'success' => true,
        'message' => 'Ansprechpartner wurde angelegt. Der Aktivierungslink kann jetzt in der Portalverwaltung sicher per E-Mail gesendet werden.',
        'id' => $contactId,
...
function company_contact_activate(string $token, string $password, string $password2): array {
    ...
    if (!$contact) {
        portal_activation_attempt_record($ip, false);
        $state = portal_activation_token_state('company_contact', $token);
    ...
    if (!empty($contact['verify_expires']) && strtotime($contact['verify_expires']) < time()) {
        portal_activation_attempt_record($ip, false);
        return ['success' => false, 'message' => 'Dieser Aktivierungslink ist abgelaufen. Bitte wenden Sie sich an MZ Tech, um einen neuen Link zu erhalten.'];
    }

    $hash = password_hash($password, PASSWORD_BCRYPT, ['cost' => BCRYPT_COST]);
    $db->prepare(
        'UPDATE company_contacts
         SET password_hash=?, password_initialized=1, is_verified=1, verified_at=NOW(), verified_by=NULL,
    ...
    $db->prepare(
        'UPDATE portal_admin_notifications
          ...
          WHERE account_type = "company_contact" AND account_id = ? AND status = "pending"'
    )->execute([$contact['id']]);
...
function company_contact_reset_password(string $token, string $password, string $password2): array {
    ...
    $db->prepare(
        'UPDATE company_contacts
         SET password_hash=?, password_initialized=1, reset_token=NULL, reset_token_hash=NULL, reset_expires=NULL, is_verified=1
         WHERE id=?'
    )
       ->execute([$hash, $contact['id']]);
...
function company_contact_change_password(int $contactId, string $current, string $new, string $new2): array {
    ...
    get_db()->prepare('UPDATE company_contacts SET password_hash = ?, password_initialized = 1 WHERE id = ?')->execute([$hash, $contactId]);
