...
function smtp_configuration_status(): array {
    ...
    $user = trim(get_setting('smtp_user', ''));
    $from = trim(get_setting('smtp_from_email', ''));
    $port = (int)get_setting('smtp_port', '587');
    $encryption = strtolower(trim(get_setting('smtp_encryption', $port === 465 ? 'smtps' : 'starttls')));
    $passwordConfigured = get_setting('smtp_pass_encrypted', '') !== '' && get_setting('smtp_pass_iv', '') !== '';
    $valid = $host !== '' && $user !== '' && $passwordConfigured
        && filter_var($from, FILTER_VALIDATE_EMAIL)
        && in_array($encryption, ['starttls', 'smtps'], true)
        && $port >= 1 && $port <= 65535;
    return [
        'configured' => $valid,
        'tls' => in_array($encryption, ['starttls', 'smtps'], true),
        'host_configured' => $host !== '',
        'user_configured' => $user !== '',
        'password_configured' => $passwordConfigured,
        'from_configured' => (bool)filter_var($from, FILTER_VALIDATE_EMAIL),
        'encryption' => $encryption,
        'port' => $port,
    ];
}

function send_email_result(string $to_email, string $to_name, string $subject, string $html_body, string $plain_body = '', array $attachments = []): array {
    $vendor_path = BASE_PATH . '/vendor/autoload.php';
    if (!file_exists($vendor_path)) {
        error_log('Mailer: Bibliothek fehlt.');
        return ['success' => false, 'error_category' => 'library_missing'];
    }
    ...
    if (!smtp_configuration_status()['configured']) {
        error_log('Mailer: SMTP-Konfiguration unvollständig.');
        return ['success' => false, 'error_category' => 'configuration'];
    }
    if (!filter_var($to_email, FILTER_VALIDATE_EMAIL)) {
        return ['success' => false, 'error_category' => 'recipient_invalid'];
    }
    ...
    try {
        ...
        $mail->send();
        return ['success' => true, 'error_category' => null];
    } catch (MailException $e) {
        $normalized = strtolower($mail->ErrorInfo . ' ' . $e->getMessage());
        $category = str_contains($normalized, 'authenticate') ? 'authentication'
            : (str_contains($normalized, 'connect') ? 'connection'
            : (str_contains($normalized, 'certificate') || str_contains($normalized, 'crypto') ? 'tls'
            : (str_contains($normalized, 'recipient') || str_contains($normalized, 'address') ? 'recipient' : 'smtp')));
        error_log('Mailer: Versand fehlgeschlagen (' . $category . ').');
...
function send_email(string $to_email, string $to_name, string $subject, string $html_body, string $plain_body = '', array $attachments = []): bool {
    return send_email_result($to_email, $to_name, $subject, $html_body, $plain_body, $attachments)['success'];
}

function send_portal_activation_email(string $accountType, array $account, string $activationLink, int $lifetimeHours): array {
    if (!in_array($accountType, ['customer', 'company_contact'], true)
        || !preg_match('#^https://#i', $activationLink)) {
        return ['success' => false, 'error_category' => 'request_invalid'];
    }
    ...
    if ($recipientName === '') $recipientName = trim((string)($account['company_name'] ?? 'Kunde'));
    $isCompany = $accountType === 'company_contact';
    $subject = $isCompany
        ? 'Ihr Zugang zum MZ-Tech-Firmenportal'
        : 'Ihr Zugang zum MZ-Tech-Kundenportal';
    $companyName = trim(get_setting('company_name', 'MZ Tech')) ?: 'MZ Tech';
    $address = trim(get_setting('company_address', ''));
    $phone = trim(get_setting('company_phone', ''));
    $contactEmail = trim(get_setting('company_email', get_setting('smtp_from_email', '')));
    $website = trim(get_setting('company_website', 'https://mztech-it.de/'));
    $greeting = $firstName !== '' ? $firstName : ($recipientName !== '' ? $recipientName : 'Kunde');
    $portalName = $isCompany ? 'MZ-Tech-Firmenportal' : 'MZ-Tech-Kundenportal';
    $features = $isCompany
        ? ['Reparaturstatus und Projekte ansehen', 'Angebote und Rechnungen einsehen', 'Dokumente herunterladen', 'Support-Tickets und Nachrichten verwalten']
        : ['Reparaturstatus ansehen', 'Angebote und Rechnungen einsehen', 'Dokumente herunterladen', 'Nachrichten und Informationen zu Ihrem Auftrag abrufen'];
    $featureHtml = implode('', array_map(static fn(string $item): string => '<li style="margin:6px 0;">' . h($item) . '</li>', $features));
    $contactLines = array_values(array_filter([$address, $phone, $contactEmail, $website], static fn(string $value): bool => $value !== ''));
    $contactHtml = implode('<br>', array_map('h', $contactLines));
    $plainContact = implode("\n", $contactLines);
    $hours = max(1, min(168, $lifetimeHours));
    $safeLink = h($activationLink);
    $html = '<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"></head>'
        . '<body style="margin:0;background:#f3f5f7;font-family:Arial,sans-serif;color:#1f2933;">'
        . '<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f3f5f7;padding:20px 10px;"><tr><td align="center">'
        ...
        . '<tr><td style="background:#0057b8;color:#fff;padding:24px;font-size:24px;font-weight:bold;">' . h($companyName) . '</td></tr>'
        . '<tr><td style="padding:28px 24px;"><p style="margin-top:0;">Hallo ' . h($greeting) . ',</p>'
        . '<p>für Ihre E-Mail-Adresse wurde ein Zugang zum ' . h($portalName) . ' eingerichtet.</p>'
        . '<p>Bitte bestätigen Sie Ihren Zugang über die folgende Schaltfläche:</p>'
        . '<p style="text-align:center;margin:28px 0;"><a href="' . $safeLink . '" style="display:inline-block;background:#0057b8;color:#fff;text-decoration:none;padding:14px 22px;border-radius:6px;font-weight:bold;">Konto jetzt aktivieren</a></p>'
        . '<p>Der Aktivierungslink ist ' . $hours . ' Stunden gültig und kann nur einmal verwendet werden.</p>'
        . '<p>Nach der Aktivierung können Sie unter anderem:</p><ul style="padding-left:22px;">' . $featureHtml . '</ul>'
        . '<p>Falls Sie diesen Zugang nicht angefordert haben, ignorieren Sie diese Nachricht oder kontaktieren Sie MZ Tech.</p>'
        . '<p style="font-size:13px;color:#52606d;word-break:break-all;">Falls die Schaltfläche nicht funktioniert, öffnen Sie folgenden Link im Browser:<br><a href="' . $safeLink . '">' . $safeLink . '</a></p>'
        . '<p>Freundliche Grüße<br><strong>MZ Tech</strong><br>Technik · Reparatur · Service</p>'
        . ($contactHtml !== '' ? '<p style="font-size:13px;color:#52606d;">' . $contactHtml . '</p>' : '')
        . '</td></tr></table></td></tr></table></body></html>';
    $plain = "Hallo {$greeting},\n\nfür Ihre E-Mail-Adresse wurde ein Zugang zum {$portalName} eingerichtet.\n\n"
        . "Konto jetzt aktivieren:\n{$activationLink}\n\nDer Aktivierungslink ist {$hours} Stunden gültig und kann nur einmal verwendet werden.\n\n"
        . "Nach der Aktivierung können Sie unter anderem:\n- " . implode("\n- ", $features) . "\n\n"
        . "Falls Sie diesen Zugang nicht angefordert haben, ignorieren Sie diese Nachricht oder kontaktieren Sie MZ Tech.\n\n"
        . "Freundliche Grüße\nMZ Tech\nTechnik · Reparatur · Service"
        . ($plainContact !== '' ? "\n{$plainContact}" : '');
    return send_email_result($email, $recipientName, $subject, $html, $plain);
}
