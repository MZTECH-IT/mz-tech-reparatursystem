...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_GET['action'] ?? '') === 'save') {
    ...
    $markup = '10.00';
    $calculated = null;
    $manual_price = isset($_POST['manual_selling_price']) && ($_POST['selling_price'] ?? '') !== '';
    $sell_price = null;
    try {
        $purch_price = ($_POST['purchase_price'] ?? '') !== ''
            ? repair_decimal_input($_POST['purchase_price'], 'Einkaufspreis', '9999999.99')
            : null;
        $markup = repair_decimal_input($_POST['markup_percent'] ?? '10.00', 'Aufschlag', '1000.00');
        $calculated = $purch_price !== null ? part_sales_price($purch_price, $markup) : null;
        $sell_price = $manual_price
            ? repair_decimal_input($_POST['selling_price'], 'Verkaufspreis', '9999999.99')
            : ($calculated['automatic_selling_price'] ?? null);
    ...
    if (empty($errors)) {
        if ($id > 0) {
            ...
            $stmt = $db->prepare('UPDATE parts SET sku=?, ean=?, mpn=?, name=?, category=?, subcategory=?, device_type=?,
                ...
                stock_quantity=?, min_stock=?, purchase_price=?, markup_percent=?, automatic_selling_price=?,
                selling_price=?, selling_price_manual=?, selling_price_changed_by=?, selling_price_changed_at=IF(?, NOW(), selling_price_changed_at)
                WHERE id=?');
            $stmt->execute([$sku, $ean, $mpn, $name, $category, $subcategory, $device_type,
                $model_compatibility, $brand, $manufacturer, $description, $quality_tier, $is_discontinued,
                $stock_location, $preferred_supplier_id,
                $stock_qty, $min_stock, $purch_price, $markup,
                $calculated['automatic_selling_price'] ?? null, $sell_price, $manual_price ? 1 : 0,
                $manual_price ? ($_SESSION['user_id'] ?? null) : null, $manual_price ? 1 : 0, $id]);
            ...
            $stmt = $db->prepare('INSERT INTO parts (sku, ean, mpn, name, category, subcategory, device_type,
                ...
                stock_location, preferred_supplier_id,
                stock_quantity, min_stock, purchase_price, markup_percent, automatic_selling_price,
                selling_price, selling_price_manual, selling_price_changed_by, selling_price_changed_at)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,IF(?,NOW(),NULL))');
            $stmt->execute([$sku, $ean, $mpn, $name, $category, $subcategory, $device_type,
                $model_compatibility, $brand, $manufacturer, $description, $quality_tier, $is_discontinued,
                $stock_location, $preferred_supplier_id,
                $stock_qty, $min_stock, $purch_price, $markup,
                $calculated['automatic_selling_price'] ?? null, $sell_price, $manual_price ? 1 : 0,
                $manual_price ? ($_SESSION['user_id'] ?? null) : null, $manual_price ? 1 : 0]);
    ...
    $edit_part  = [
        ...
        'purchase_price' => $purch_price, 'markup_percent' => $markup,
        'automatic_selling_price' => $calculated['automatic_selling_price'] ?? null,
        'selling_price' => $sell_price, 'selling_price_manual' => $manual_price ? 1 : 0,
    ];
}
        ...
        <form method="post" action="parts.php?action=save">
            ...
            <div class="modal-body">
                ...
                <div class="form-grid" style="grid-template-columns:1fr 1fr;gap:1rem;">
                    ...
                    <div class="form-group">
                        <label for="p-markup">Aufschlag (%)</label>
                        <input type="number" id="p-markup" name="markup_percent"
                               value="<?= h($edit_part['markup_percent'] ?? '10.00') ?>" step="0.01" min="0" max="1000">
                    </div>
                    <div class="form-group">
                        <label for="p-autoprice">Automatischer VK-Preis (€)</label>
                        <input type="text" id="p-autoprice" readonly value="<?= h($edit_part['automatic_selling_price'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        ...
                        <label style="display:block;margin-top:.35rem;"><input type="checkbox" name="manual_selling_price" value="1" <?= !empty($edit_part['selling_price_manual']) ? 'checked' : '' ?>> manueller Verkaufspreis</label>
                    </div>
                </div>
            </div>
...
function openPartModal(data) {
    ...
    form.querySelector('[name="markup_percent"]').value  = data ? (data.markup_percent || 10) : 10;
    form.querySelector('[name="manual_selling_price"]').checked = !!(data && Number(data.selling_price_manual));
...
function updateCalculatedPartPrice() {
    var purchase = Number(String(document.getElementById('p-ekpreis').value || '0').replace(',', '.'));
    var markup = Number(String(document.getElementById('p-markup').value || '10').replace(',', '.'));
    var calculated = Math.round((purchase * (1 + markup / 100) + Number.EPSILON) * 100) / 100;
    document.getElementById('p-autoprice').value = calculated.toFixed(2).replace('.', ',');
    var manual = document.querySelector('[name="manual_selling_price"]').checked;
    if (!manual) document.getElementById('p-vkpreis').value = calculated.toFixed(2);
}
document.getElementById('p-ekpreis').addEventListener('input', updateCalculatedPartPrice);
document.getElementById('p-markup').addEventListener('input', updateCalculatedPartPrice);
document.querySelector('[name="manual_selling_price"]').addEventListener('change', updateCalculatedPartPrice);
