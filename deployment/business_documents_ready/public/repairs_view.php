...
$available_parts = [];
if (user_has_permission('manage_parts') && empty($repair['invoice_number'])) {
    $available_parts = $db->query(
        'SELECT id,sku,name,stock_quantity,selling_price FROM parts WHERE stock_quantity>0 ORDER BY name LIMIT 500'
    ...
    <div style="padding:4px 4px 12px;display:flex;flex-wrap:wrap;gap:10px;align-items:center;">
        ...
        <div style="margin-left:auto;display:flex;gap:8px;flex-wrap:wrap;">
            <?php if (user_has_permission('manage_payments')): ?>
                <a href="<?= url('payments.php') ?>?repair_id=<?= (int)$id ?>" class="btn btn-outline btn-sm"><?= svg_icon('euro',15) ?> Zahlungen</a>
            <?php endif; ?>
...
<div class="detail-grid">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <dl class="detail-list">
                <dt>Geräteart</dt>
                <dd><?= h(device_type_label((string)($repair['device_type'] ?? ''))) ?></dd>
        ...
        <div class="card-body">
            <dl class="detail-list">
                ...
                <?php if (!empty($repair['performed_work'])): ?>
                    <dt>Durchgeführte Arbeiten</dt>
                    <dd style="white-space:pre-wrap;"><?= h($repair['performed_work']) ?></dd>
                <?php endif; ?>

                <dt>Arbeitszeit</dt>
                <dd><?= h(number_format((float)($repair['working_hours'] ?? 0), 2, ',', '.')) ?> Stunden</dd>

                <dt>Stundensatz netto</dt>
                <dd><?= h(fmt_money((float)($repair['hourly_rate'] ?? 0))) ?></dd>

                <dt>Arbeitskosten netto</dt>
                <dd><?= h(fmt_money((float)($repair['labor_cost'] ?? 0))) ?></dd>

                <?php if ($repair['internal_notes']): ?>
                    <dt>Interne Notizen – nicht für Kunden sichtbar</dt>
                    <dd style="white-space:pre-wrap;"><?= h($repair['internal_notes']) ?></dd>
                <?php endif; ?>

                <dt>Zusätzlicher Servicepreis</dt>
...
<!-- ── Verwendete Teile ───────────────────────────────────────────────────── -->
<?php if (!empty($repair_parts) || !empty($available_parts)): ?>
<div class="card" style="margin-top:1.25rem;">
    <div class="card-header">
        <h3 class="card-title"><?= svg_icon('package', 18) ?> Verwendete Ersatzteile</h3>
    </div>
    <div class="card-body">
        <?php if ($available_parts): ?>
        <form id="add-repair-part" style="display:flex;gap:.5rem;align-items:flex-end;flex-wrap:wrap;margin-bottom:1rem;">
            <div class="form-group" style="margin:0;min-width:280px;flex:1"><label>Ersatzteil</label><select name="part_id" required><option value="">Bitte auswählen</option><?php foreach($available_parts as $part): ?><option value="<?= (int)$part['id'] ?>"><?= h($part['name']) ?><?= $part['sku']?' · '.h($part['sku']):'' ?> · Bestand <?= (int)$part['stock_quantity'] ?> · <?= h(fmt_money((float)$part['selling_price'])) ?></option><?php endforeach; ?></select></div>
            <div class="form-group" style="margin:0;width:100px"><label>Menge</label><input type="number" name="quantity" value="1" min="1" required></div>
            <button class="btn btn-outline" type="submit"><?= svg_icon('plus',15) ?> Hinzufügen</button>
        </form>
        <?php endif; ?>
        <?php if ($repair_parts): ?>
        ...
        <?php else: ?><p class="text-muted">Noch keine Ersatzteile zugeordnet.</p><?php endif; ?>
    </div>
</div>
<?php endif; ?>

<?php if ($available_parts): ?><script>
document.getElementById('add-repair-part').addEventListener('submit', function (event) {
    event.preventDefault();
    var body = new URLSearchParams(new FormData(this));
    body.set('action','add_part'); body.set('repair_id','<?= (int)$id ?>'); body.set('csrf_token',<?= json_encode(csrf_token()) ?>);
    fetch(<?= json_encode(url('api/repairs.php')) ?>,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:body.toString()})
      .then(function(r){return r.json().then(function(data){return {ok:r.ok,data:data};});})
      .then(function(result){if(!result.ok||!result.data.success) throw new Error(result.data.message||'Fehler'); location.reload();})
