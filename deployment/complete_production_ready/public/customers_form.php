...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['portal_action']) && $is_edit) {
    ...
    $new_portal_access = ensure_customer_portal_access($id, $regenerate);
    if (!empty($new_portal_access['link'])) {
        $_SESSION['portal_access_once'] = [
            'customer_id' => $id,
            'link' => $new_portal_access['link'],
            'pin' => $new_portal_access['pin'],
        ];
    }
...
<div class="card" style="margin-top: 1.5rem;" id="portal-access">
    ...
    <div class="card-body">
        ...
        $portal_access_once = $_SESSION['portal_access_once'] ?? null;
        if (($portal_access_once['customer_id'] ?? 0) === $id) {
            unset($_SESSION['portal_access_once']);
        } else {
            $portal_access_once = null;
        }
        $portal_pin = null;
        ...
        <?php else: ?>
            <?php if ($portal_access_once): ?>
                <div class="alert alert-warning">
                    Dieser Link und die PIN werden aus Sicherheitsgründen nur jetzt angezeigt.
                    <div class="form-group full" style="margin-top:10px;">
                        <label for="portal_link_once">Neuer persönlicher Zugangslink</label>
                        <input type="text" id="portal_link_once" readonly
                               value="<?= h($portal_access_once['link']) ?>" onclick="this.select()">
                    </div>
                    <div class="form-group">
                        <label for="portal_pin_once">Neue PIN</label>
                        <input type="text" id="portal_pin_once" readonly
                               value="<?= h($portal_access_once['pin']) ?>" onclick="this.select()">
                    </div>
                </div>
            <?php endif; ?>
            <div class="form-grid">
                <div class="form-group full">
                    <label>Persönlicher Zugangslink</label>
                    <input type="text" readonly value="Aus Sicherheitsgründen nicht erneut abrufbar">
                </div>
                <div class="form-group">
            ...
            <div style="display:flex;align-items:center;gap:20px;flex-wrap:wrap;margin-top:12px;">
                <div>
                    <p class="form-hint" style="margin-top:0;">
                        Erzeugen Sie bei Bedarf einen neuen Link. Der bisherige Link wird dabei
                        widerrufen; Klartext-Tokens werden nicht in der Datenbank gespeichert.
                    </p>
                    <form method="post" action="customers_form.php?id=<?= $id ?>"
