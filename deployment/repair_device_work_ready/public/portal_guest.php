...
if ($access) {
    if ($access['scope_type'] === 'repair') {
        $stmt = get_db()->prepare(
            'SELECT id, repair_number, device_type, manufacturer, model, status, estimated_ready,
                    performed_work, working_hours, hourly_rate, labor_cost, updated_at
...
<main style="max-width:760px;margin:40px auto;padding:0 20px;">
  <div class="card"><div class="card-body">
    ...
    <?php elseif ($access['scope_type'] === 'repair'): ?>
      ...
      <p><strong>Gerät:</strong> <?= h(trim(($record['manufacturer'] ?? '') . ' ' . ($record['model'] ?? '')) ?: device_type_label((string)$record['device_type'])) ?></p>
      ...
      <?php if (!empty($record['performed_work'])): ?>
        <p><strong>Durchgeführte Arbeiten:</strong></p>
        <div style="white-space:pre-wrap;"><?= h($record['performed_work']) ?></div>
      <?php endif; ?>
      <?php if (!empty($record['labor_cost'])): ?>
        <p><strong>Arbeitsleistung netto:</strong> <?= h(repair_labor_description($record)) ?></p>
      <?php endif; ?>
