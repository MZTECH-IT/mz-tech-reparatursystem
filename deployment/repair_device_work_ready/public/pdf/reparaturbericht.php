...
// Kundengeeignete Leistungsbeschreibung; interne Notizen werden absichtlich
// weder gelesen noch an das Kundendokument übergeben.
if (!empty($repair['performed_work']) || (float)($repair['labor_cost'] ?? 0) > 0.0) {
    ...
    if ((float)($repair['working_hours'] ?? 0) > 0.0 || (float)($repair['labor_cost'] ?? 0) > 0.0) {
        $drawRow($pdf, 'Arbeitszeit', number_format((float)($repair['working_hours'] ?? 0), 2, ',', '.') . ' Stunden');
        $drawRow($pdf, 'Stundensatz netto', number_format((float)($repair['hourly_rate'] ?? 0), 2, ',', '.') . ' €');
        $drawRow($pdf, 'Arbeitskosten netto', number_format((float)($repair['labor_cost'] ?? 0), 2, ',', '.') . ' €');
    }
    $pdf->SetY($pdf->GetY() + 3);
}
