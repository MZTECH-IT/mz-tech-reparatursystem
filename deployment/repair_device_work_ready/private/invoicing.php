...
function invoice_validate_for_release(int $repairId): array {
    ...
    $stmt = $db->prepare(
        'SELECT r.*, c.first_name, c.last_name, c.address, c.email, c.phone
    ...
    $repairPrice = (float)($repair['price'] ?? 0);
    $laborCost = (float)($repair['labor_cost'] ?? 0);
    $total = $repairPrice + $laborCost + $partsTotal; // Anzahlung mindert nur den offenen Betrag
    if ($total <= 0) {
        $warnings[] = 'Der Rechnungsbetrag ist 0,00 € — bitte prüfen, ob das beabsichtigt ist (z. B. Garantie/Kulanz).';
    }

    // 4) Steuerlogik
    $taxRate = (float)str_replace(',', '.', get_setting('tax_rate', '0'));
...
function invoice_release(int $repairId, ?int $userId): array {
    ...
    try {
        ...
        $snapshot = build_document_snapshot([
            ...
            'ustg_active'    => function_exists('pdf_ustg_active') ? pdf_ustg_active() : false,
            'ustg_notice'    => function_exists('pdf_ustg_notice_text') ? pdf_ustg_notice_text() : '',
...
function quote_of_repair_release(int $repairId, ?int $userId): array {
    ...
    try {
        ...
        if (empty($number)) {
            ...
            $db->prepare('UPDATE repairs SET quote_number = ? WHERE id = ?')->execute([$number, $repairId]);
        }
        $db->prepare('UPDATE repairs SET quote_status = "freigegeben" WHERE id = ?')->execute([$repairId]);
