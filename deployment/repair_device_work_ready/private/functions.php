...
// ── Gerätearten (zentrale Stammdaten + migrationssicherer Fallback) ──────
function device_type_default_records(): array {
    return [
        ['technical_key' => 'smartphone',       'display_name' => 'Smartphone',            'category' => 'Mobilgeräte',            'sort_order' => 10],
        ['technical_key' => 'tablet',           'display_name' => 'Tablet',                'category' => 'Mobilgeräte',            'sort_order' => 20],
        ['technical_key' => 'pc',               'display_name' => 'PC',                    'category' => 'Computer',               'sort_order' => 30],
        ['technical_key' => 'laptop',           'display_name' => 'Laptop',                'category' => 'Computer',               'sort_order' => 40],
        ['technical_key' => 'fernseher',        'display_name' => 'Fernseher',             'category' => 'Unterhaltungselektronik','sort_order' => 50],
        ['technical_key' => 'monitor',          'display_name' => 'Monitor',               'category' => 'Computer',               'sort_order' => 60],
        ['technical_key' => 'hifi_anlage',      'display_name' => 'HiFi-Anlage',           'category' => 'Unterhaltungselektronik','sort_order' => 70],
        ['technical_key' => 'verstaerker',      'display_name' => 'Verstärker',            'category' => 'Unterhaltungselektronik','sort_order' => 80],
        ['technical_key' => 'radio',            'display_name' => 'Radio',                 'category' => 'Unterhaltungselektronik','sort_order' => 90],
        ['technical_key' => 'dvd_bluray_player','display_name' => 'DVD-/Blu-ray-Player',   'category' => 'Unterhaltungselektronik','sort_order' => 100],
        ['technical_key' => 'spielkonsole',     'display_name' => 'Spielkonsole',          'category' => 'Gaming',                 'sort_order' => 110],
        ['technical_key' => 'controller',       'display_name' => 'Controller',            'category' => 'Gaming',                 'sort_order' => 120],
        ['technical_key' => 'smartwatch',       'display_name' => 'Smartwatch',            'category' => 'Mobilgeräte',            'sort_order' => 130],
        ['technical_key' => 'firmenhardware',   'display_name' => 'Firmenhardware',        'category' => 'Firmenkunden',           'sort_order' => 140],
        ['technical_key' => 'it_service',       'display_name' => 'IT-Service',            'category' => 'Dienstleistung',         'sort_order' => 150],
        ['technical_key' => 'sonstiges',        'display_name' => 'Sonstiges',             'category' => 'Sonstiges',              'sort_order' => 999],
    ];
}

function device_type_records(bool $includeInactive = false): array {
    try {
        $sql = 'SELECT id, technical_key, display_name, category, sort_order, is_active
                  FROM device_types';
        if (!$includeInactive) $sql .= ' WHERE is_active = 1';
        $sql .= ' ORDER BY CASE WHEN technical_key = "sonstiges" THEN 1 ELSE 0 END,
                         sort_order, display_name';
        $rows = get_db()->query($sql)->fetchAll(PDO::FETCH_ASSOC);
        if ($rows) return $rows;
    } catch (Throwable) {
        // Vor Ausführung der Migration bleiben alle Formulare nutzbar.
    }
    return array_map(static fn(array $row): array => $row + [
        'id' => null,
        'is_active' => 1,
    ], device_type_default_records());
}

function device_type_options(): array {
    $options = [];
    foreach (device_type_records(false) as $row) {
        $options[(string)$row['technical_key']] = (string)$row['display_name'];
    }
    return $options;
}

function device_type_key_for_value(string $value): string {
    $value = trim($value);
    if ($value === '') return '';
    $aliases = [
        'pc/desktop' => 'pc',
        ...
        'firmen-it' => 'firmenhardware',
        'firmen_it' => 'firmenhardware',
    ];
    $normalized = function_exists('mb_strtolower')
        ? mb_strtolower($value, 'UTF-8')
        : strtolower($value);
    if (isset($aliases[$normalized])) return $aliases[$normalized];
    foreach (device_type_records(true) as $row) {
        $key = (string)$row['technical_key'];
        $label = (string)$row['display_name'];
        if ($value === $key || strcasecmp($value, $label) === 0) return $key;
    }
    return $value;
}

function device_type_id_for_value(string $value): ?int {
    $key = device_type_key_for_value($value);
    try {
        $stmt = get_db()->prepare(
            'SELECT id FROM device_types
              WHERE technical_key = ? OR LOWER(display_name) = LOWER(?)
              LIMIT 1'
        );
        $stmt->execute([$key, $value]);
        $id = $stmt->fetchColumn();
        if ($id !== false) return (int)$id;
        $fallback = get_db()->query(
            'SELECT id FROM device_types WHERE technical_key = "sonstiges" LIMIT 1'
        )->fetchColumn();
...
function device_type_label(string $key): string {
    $resolved = device_type_key_for_value($key);
    foreach (device_type_records(true) as $row) {
        if ((string)$row['technical_key'] === $resolved) {
...
// ── Arbeitszeit / Arbeitskosten ─────────────────────────────────────────
function repair_decimal_input(mixed $value, string $label, string $maximum = '999999.99'): string {
    ...
    if (!preg_match('/^\d+(?:\.\d+)?$/', $raw)) {
        throw new InvalidArgumentException($label . ' ist keine gültige positive Dezimalzahl.');
    }
    $number = (float)$raw;
    if (!is_finite($number) || $number < 0 || $number > (float)$maximum) {
        throw new InvalidArgumentException($label . ' liegt außerhalb des zulässigen Bereichs.');
    }
    return number_format(round($number, 2, PHP_ROUND_HALF_UP), 2, '.', '');
}

function repair_labor_values(mixed $hoursInput, mixed $rateInput): array {
    $hours = repair_decimal_input($hoursInput, 'Arbeitszeit', '9999.99');
    $rate = repair_decimal_input($rateInput, 'Stundensatz', '999999.99');
    $hoursHundredths = (int)round((float)$hours * 100, 0, PHP_ROUND_HALF_UP);
    $rateCents = (int)round((float)$rate * 100, 0, PHP_ROUND_HALF_UP);
    $costCents = (int)round(($hoursHundredths * $rateCents) / 100, 0, PHP_ROUND_HALF_UP);
    return [
        'working_hours' => $hours,
        'hourly_rate' => $rate,
        'labor_cost' => number_format($costCents / 100, 2, '.', ''),
    ];
}

function repair_labor_description(array $repair): string {
    $hours = (float)($repair['working_hours'] ?? 0);
    $rate = (float)($repair['hourly_rate'] ?? 0);
    $cost = (float)($repair['labor_cost'] ?? 0);
    return sprintf(
        'Arbeitsleistung: %s Stunden × %s € = %s € netto',
...
function ensure_customer_portal_access(int $customer_id, bool $regenerate = false): array {
    ...
    $stmt = $db->prepare(
        'INSERT INTO customer_portal_access
            (customer_id, token, token_hash, token_last4, expires_at, revoked_at, pin_encrypted, pin_iv)
         VALUES (?,NULL,?,?,DATE_ADD(NOW(), INTERVAL 30 DAY),NULL,?,?)
         ON DUPLICATE KEY UPDATE
            token = NULL, token_hash = VALUES(token_hash), token_last4 = VALUES(token_last4),
            expires_at = VALUES(expires_at), revoked_at = NULL,
            pin_encrypted = VALUES(pin_encrypted), pin_iv = VALUES(pin_iv)'
    );
    $stmt->execute([$customer_id, $token_hash, substr($token, -4), $enc['encrypted'], $enc['iv']]);
...
function portal_link_for_customer(?int $customer_id): string {
    ...
    if (!$access) return '';
    return base_app_url() . '/portal.php?view=account_login';
}
