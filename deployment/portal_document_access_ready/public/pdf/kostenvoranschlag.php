...
if ($pdf_actor !== 'staff' && !$quote_released) {
    http_response_code(403);
    die('Dieser Kostenvoranschlag ist noch nicht für Kunden freigegeben.');
}
...
if ($quote_released && !empty($repair['quote_snapshot'])) {
    $snapshot = json_decode((string)$repair['quote_snapshot'], true);
    if (is_array($snapshot)) {
        $repair = array_merge($repair, $snapshot['repair'] ?? [], $snapshot['customer'] ?? []);
        $parts_rows = $snapshot['parts'] ?? $parts_rows;
        $company_info = $snapshot['company'] ?? $company_info;
        $billing_snapshot = $snapshot['billing'] ?? null;
    }
}
...
$billingMode = $billing_snapshot['billing_mode'] ?? billing_mode();
$amounts = billing_amounts($repair_price + $labor_cost + $parts_total, $billingMode, $billing_snapshot['tax_rate'] ?? null);
$subtotal = (float)$amounts['subtotal'];
$ustg     = billing_is_small_business($billingMode);
$tax_rate = (float)$amounts['tax_rate'];
$mwst     = (float)$amounts['tax_amount'];
$gesamt   = (float)$amounts['total'];
$ustg_notice = $billing_snapshot['legal_notice'] ?? $amounts['legal_notice'];
...
$repair_desc = device_type_label((string)($repair['device_type'] ?? 'Gerät')) . ' – ' . $problem_short;
...
if (count($parts_rows) === 0 && $repair_price <= 0.0 && $labor_cost <= 0.0) {
