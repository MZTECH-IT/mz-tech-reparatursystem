...
/**
 * Öffnet gezielt die benannte Sitzung aus ihrem eigenen Cookie. Auf einer
 * PDF-URL können Admin-, Kunden- und Firmen-Cookie gleichzeitig vorhanden
 * sein. PHP kann pro Request aber nur eine Session aktiv halten. Deshalb
 * muss eine erfolglose Prüfung die Session sauber schließen, bevor der
 * nächste Cookie geprüft wird; andernfalls bliebe z. B. eine abgelaufene
 * Admin-Session aktiv und das Kundenportal könnte nie geladen werden.
 */
function pdf_open_named_session(string $sessionName, string $bootstrapFile, string $starter): bool {
    $cookieSessionId = (string)($_COOKIE[$sessionName] ?? '');
    if ($cookieSessionId === '' || !preg_match('/\A[A-Za-z0-9,-]+\z/', $cookieSessionId)) return false;
    ...
    if (session_status() !== PHP_SESSION_ACTIVE) {
        $_SESSION = [];
        // Die konkrete Sitzung explizit setzen. Ein vorheriger Aufruf von
        // session_id('') markiert die ID in einigen produktiven PHP-
        // Konfigurationen bereits als bewusst gesetzt; session_start() liest
        // dann das gleichnamige Cookie nicht mehr ein und erzeugt eine neue,
        // leere Sitzung. Das führte trotz gültiger Anmeldung zu einem 403.
        session_name($sessionName);
        session_id($cookieSessionId);
        $starter();
    }
    return session_status() === PHP_SESSION_ACTIVE && session_name() === $sessionName;
}

/** Liefert die aktive, serverseitig validierte Firmen-ID oder null. */
function pdf_authenticated_business_company_id(): ?int {
    if (!pdf_open_named_session(
        BUSINESS_SESSION_NAME,
        PRIVATE_PATH . '/business_auth.php',
    ...
    return $stmt->fetchColumn() ? $companyId : null;
}

/** Liefert die aktive, serverseitig validierte Kunden-ID oder null. */
function pdf_authenticated_portal_customer_id(): ?int {
    if (!pdf_open_named_session(
        PORTAL_SESSION_NAME,
        PRIVATE_PATH . '/portal_auth.php',
        'start_portal_session'
    )) return null;

    $customerId = portal_current_customer_id();
    if (!$customerId || !portal_is_logged_in()) return null;

    $accountId = function_exists('portal_current_account_id') ? portal_current_account_id() : null;
    if ($accountId) {
        $stmt = get_db()->prepare(
            'SELECT 1 FROM customer_accounts
             WHERE id = ? AND customer_id = ? AND is_active = 1 AND is_verified = 1
             LIMIT 1'
        );
        $stmt->execute([$accountId, $customerId]);
        return $stmt->fetchColumn() ? $customerId : null;
    }

    $accessId = (int)($_SESSION['portal_access_id'] ?? 0);
    if ($accessId < 1) return null;
    $stmt = get_db()->prepare(
    ...
    $stmt->execute([$accessId, $customerId]);
    return $stmt->fetchColumn() ? $customerId : null;
}

/** Ermittelt den autorisierten Akteur für eine Reparatur oder null. */
function pdf_repair_access_actor(int $repairId): ?string {
    if ($repairId < 1) return null;

    if (pdf_open_named_session(SESSION_NAME, PRIVATE_PATH . '/auth.php', 'start_secure_session')
        && !empty($_SESSION['user_id'])) {
        return 'staff';
    }

    $companyId = pdf_authenticated_business_company_id();
    if ($companyId) {
        $stmt = get_db()->prepare(
        ...
        $stmt->execute([$repairId, $companyId, $companyId]);
        if ($stmt->fetchColumn()) return 'business';
    }

    $customerId = pdf_authenticated_portal_customer_id();
    if ($customerId) {
        $stmt = get_db()->prepare(
            'SELECT 1 FROM repairs WHERE id = ? AND customer_id = ? LIMIT 1'
        );
        $stmt->execute([$repairId, $customerId]);
        if ($stmt->fetchColumn()) return 'portal';
    }
...
/**
 * Autorisiert Mitarbeiter, den Eigentümer im Kundenportal oder einen
 * aktiven Firmenkontakt derselben Firma. Rückgabe wird von einzelnen
 * Dokumenttypen zusätzlich für deren Freigabestatus verwendet.
 */
function pdf_authorize_repair_access(int $repairId): string {
    $actor = pdf_repair_access_actor($repairId);
...
function pdf_quote_access_actor(array $quote): ?string {
    if (pdf_open_named_session(SESSION_NAME, PRIVATE_PATH . '/auth.php', 'start_secure_session')
        && !empty($_SESSION['user_id'])) return 'staff';
    ...
    $companyId = pdf_authenticated_business_company_id();
    if ($companyId && !empty($quote['company_id']) && $companyId === (int)$quote['company_id']) {
        return 'business';
    }

    $customerId = pdf_authenticated_portal_customer_id();
    if ($customerId && !empty($quote['customer_id']) && $customerId === (int)$quote['customer_id']) {
        return 'portal';
    }
...
/** Autorisiert ein eigenständiges, für Kunden freigegebenes Angebot. */
function pdf_authorize_quote_access(array $quote): string {
    $actor = pdf_quote_access_actor($quote);
    if ($actor !== null) return $actor;
