...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'release') {
        ...
        purchase_order_set_status($id, 'storniert');
        flash('success', 'Bestellung wurde storniert.');
    } elseif ($action === 'send_email') {
        $result = purchase_order_send_email($id, $_SESSION['user_id'] ?? null);
        flash($result['success'] ? 'success' : 'error', $result['message']);
    }
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <?php if (in_array($order['status'], ['bestellt', 'teilweise_geliefert', 'geliefert'], true)): ?>
        <div class="action-group" style="margin-top:.75rem;">
            <a href="<?= url('pdf/bestellung.php') ?>?id=<?= (int)$id ?>" target="_blank" class="btn btn-outline"><?= svg_icon('pdf', 16) ?> PDF anzeigen</a>
            <form method="post" class="inline-form" onsubmit="return confirm('Bestellung per E-Mail an den Lieferanten senden?');">
                <?= csrf_field() ?><input type="hidden" name="action" value="send_email">
                <button type="submit" class="btn btn-outline"><?= svg_icon('mail', 16) ?> An Lieferant senden</button>
            </form>
            <a href="<?= url('purchase_order_receive.php') ?>?id=<?= (int)$id ?>" class="btn btn-outline"><?= svg_icon('upload', 16) ?> Wareneingang erfassen</a>
        </div>
        <?php endif; ?>
        <?php if (!in_array($order['status'], ['storniert', 'geliefert'], true)): ?>
        <form method="post" class="inline-form" style="margin-top:.5rem;" onsubmit="return confirm('Diese Bestellung wirklich stornieren?');">
            <?= csrf_field() ?><input type="hidden" name="action" value="cancel">
            <button type="submit" class="btn btn-danger"><?= svg_icon('x', 16) ?> Stornieren</button>
        </form>
        <?php endif; ?>

        <div class="table-wrap" style="margin-top:1rem;">
            <table class="table">
                <thead><tr><th>SKU</th><th>Name</th><th class="text-right">Menge</th><th class="text-right">Preis (eingefroren)</th><th>Zuordnung</th><th class="text-right">Erhalten</th><?php if (in_array($order['status'], ['bestellt', 'teilweise_geliefert'], true)): ?><th class="col-actions">Wareneingang</th><?php endif; ?></tr></thead>
                <tbody>
                <?php foreach ($items as $it): ?>
                    <tr>
                        ...
                        <?php if (in_array($order['status'], ['bestellt', 'teilweise_geliefert'], true)): ?>
                        <td class="col-actions">
                            <form method="post" style="display:flex;gap:.4rem;">
...
<div class="card">
    <div class="card-header"><h2 class="card-title"><?= svg_icon('list', 20) ?> Statushistorie</h2></div>
    <div class="card-body">
        <?php $history = purchase_order_status_history($id); ?>
        <?php if (empty($history)): ?>
            <p class="text-muted">Noch keine protokollierten Statusänderungen.</p>
        <?php else: ?>
        <div class="table-wrap">
            <table class="table">
                <thead><tr><th>Datum</th><th>Benutzer</th><th>Aktion</th><th>Details</th></tr></thead>
                <tbody>
                <?php foreach ($history as $h): ?>
                    <tr>
                        <td><?= h(date('d.m.Y H:i', strtotime($h['created_at']))) ?></td>
                        <td><?= h($h['user_name'] ?? 'System') ?></td>
                        <td><?= h($h['action']) ?></td>
                        <td class="text-muted" style="font-size:.85rem;"><?= h($h['details'] ?? '') ?></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
