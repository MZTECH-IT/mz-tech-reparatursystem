...
function business_login_as(int $contactId, int $companyId): void {
...
function business_attempt_login(string $email, string $password): array {
    ...
    if (!(int)$contact['is_verified']) {
        business_record_login_attempt($ip, $email, false);
        return ['success' => false, 'message' => 'Bitte aktivieren Sie zuerst Ihren Zugang über den Link in Ihrer Einladungsmail.'];
    }

    business_record_login_attempt($ip, $email, true);
    business_login_as((int)$contact['id'], (int)$contact['company_id']);
