 ...
 * (Admin-Session) ODER als angemeldeter Kundenportal-Nutzer, dem genau
 * diese Reparatur gehört (Gast-Zugang oder registriertes Konto – beide
 * laufen in derselben Portal-Session, siehe portal_login_as()).
 *
...
function pdf_authorize_repair_access(int $repairId): void {
    ...
    // 2) Kundenportal – Gast-Zugang ODER registriertes Konto, aber nur
...
function pdf_ustg_active(): bool {
    $rate = get_setting('tax_rate', '0');
    return (float)str_replace(',', '.', $rate) <= 0.0;
}
