...
function customer_register(array $data): array {
    ...
    try {
        $stmt = $db->prepare(
            'INSERT INTO customer_accounts (customer_id, email, password_hash, is_verified, verify_token, verify_expires)
             VALUES (?, ?, ?, 0, ?, DATE_ADD(NOW(), INTERVAL 24 HOUR))'
        );
        $stmt->execute([$customer_id, $email, $hash, $verify_token]);
    ...
    try {
        $verify_link = base_app_url() . '/portal.php?verify=' . $verify_token;
    ...
    return [
        'success' => true,
        'message' => 'Registrierung erfolgreich! Bitte bestätigen Sie Ihre E-Mail-Adresse über den Link, den wir Ihnen gerade per E-Mail gesendet haben.',
    ];
}
...
function customer_verify_email(string $token): array {
    ...
    $db = get_db();
    $stmt = $db->prepare('SELECT * FROM customer_accounts WHERE verify_token = ? LIMIT 1');
    $stmt->execute([$token]);
    $account = $stmt->fetch();

    if (!$account) {
        return ['success' => false, 'message' => 'Ungültiger oder bereits verwendeter Bestätigungslink.'];
    }
    ...
    $db->prepare('UPDATE customer_accounts SET is_verified = 1, verify_token = NULL, verify_expires = NULL WHERE id = ?')
       ->execute([$account['id']]);

    log_activity('customer_verify_email', 'customer_accounts', (int)$account['id'], 'E-Mail-Adresse bestätigt');

    return ['success' => true, 'message' => 'Ihre E-Mail-Adresse wurde erfolgreich bestätigt. Sie können sich jetzt anmelden.'];
}
...
function customer_attempt_login(string $email, string $password): array {
    ...
    if (!(int)$account['is_verified']) {
        portal_record_login_attempt($ip, $email, false);
        return ['success' => false, 'message' => 'Bitte bestätigen Sie zuerst Ihre E-Mail-Adresse (Link in der Registrierungsmail).'];
    }
...
function customer_request_password_reset(string $email): array {
    $email = trim($email);
    $generic = ['success' => true, 'message' => 'Falls zu dieser E-Mail-Adresse ein Konto besteht, wurde soeben ein Link zum Zurücksetzen des Passworts versendet.'];
    ...
    if ($account) {
        ...
        $db = get_db();
        $db->prepare('UPDATE customer_accounts SET reset_token = ?, reset_expires = DATE_ADD(NOW(), INTERVAL 60 MINUTE) WHERE id = ?')
        ...
        try {
            $reset_link = base_app_url() . '/portal.php?reset=' . $token;
...
function customer_reset_password(string $token, string $password, string $password2): array {
    ...
    $db = get_db();
    $stmt = $db->prepare('SELECT * FROM customer_accounts WHERE reset_token = ? LIMIT 1');
    $stmt->execute([$token]);
    $account = $stmt->fetch();
    ...
    $hash = password_hash($password, PASSWORD_BCRYPT, ['cost' => BCRYPT_COST]);
    $db->prepare('UPDATE customer_accounts SET password_hash = ?, reset_token = NULL, reset_expires = NULL, is_verified = 1 WHERE id = ?')
