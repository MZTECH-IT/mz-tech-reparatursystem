...
function company_contact_create(int $companyId, array $data): array {
    ...
    $stmt = $db->prepare(
        'INSERT INTO company_contacts
            (company_id, first_name, last_name, email, phone, role_title, password_hash, is_primary, is_active, is_verified, verify_token, verify_expires)
         VALUES (?,?,?,?,?,?,?,?,1,0,?,DATE_ADD(NOW(), INTERVAL 7 DAY))'
    );
    $stmt->execute([
    ...
    try {
        ...
        $verify_link = base_app_url() . '/portal_business.php?activate=' . $token;
    ...
    return ['success' => true, 'message' => 'Ansprechpartner wurde angelegt. Eine E-Mail zur Kontoaktivierung wurde versendet.', 'id' => $contactId];
}

function company_contact_update(int $id, array $data): void {
    get_db()->prepare(
        'UPDATE company_contacts SET first_name=?, last_name=?, phone=?, role_title=?, is_primary=?, is_active=? WHERE id=?'
    )->execute([
...
function company_contact_activate(string $token, string $password, string $password2): array {
    ...
    $db = get_db();
    $stmt = $db->prepare('SELECT * FROM company_contacts WHERE verify_token = ? LIMIT 1');
    $stmt->execute([$token]);
    ...
    $db->prepare('UPDATE company_contacts SET password_hash=?, is_verified=1, verify_token=NULL, verify_expires=NULL WHERE id=?')
...
function company_contact_request_password_reset(string $email): array {
    $email = trim($email);
    $generic = ['success' => true, 'message' => 'Falls zu dieser E-Mail-Adresse ein Zugang besteht, wurde soeben ein Link zum Zurücksetzen des Passworts versendet.'];
    ...
    if ($contact && (int)$contact['is_active']) {
        $token = bin2hex(random_bytes(32));
        get_db()->prepare('UPDATE company_contacts SET reset_token=?, reset_expires=DATE_ADD(NOW(), INTERVAL 60 MINUTE) WHERE id=?')
            ->execute([$token, $contact['id']]);
        try {
            $reset_link = base_app_url() . '/portal_business.php?reset=' . $token;
...
function company_contact_reset_password(string $token, string $password, string $password2): array {
    ...
    $db = get_db();
    $stmt = $db->prepare('SELECT * FROM company_contacts WHERE reset_token = ? LIMIT 1');
    $stmt->execute([$token]);
    $contact = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    $hash = password_hash($password, PASSWORD_BCRYPT, ['cost' => BCRYPT_COST]);
    $db->prepare('UPDATE company_contacts SET password_hash=?, reset_token=NULL, reset_expires=NULL, is_verified=1 WHERE id=?')
       ->execute([$hash, $contact['id']]);
