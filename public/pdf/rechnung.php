...
if ($snapshot !== null) {
    ...
    $tax_rate       = (float)($snapshot['tax_rate'] ?? 0);
    $ustg           = (bool)($snapshot['ustg_active'] ?? false);
    $ustg_notice    = $snapshot['ustg_notice'] ?? pdf_ustg_notice_text();
    ...
    $cust_address   = $customer_snap['address']    ?? $repair['address'];
    $cust_phone     = $customer_snap['phone']      ?? $repair['phone'];
    $cust_email     = $customer_snap['email']      ?? $repair['email'];
    $draftLabel     = null;
} else {
    ...
    $cust_address   = $repair['address'];
    $cust_phone     = $repair['phone'];
    $cust_email     = $repair['email'];
...
$device_source  = $snapshot !== null ? ($repair_snap['device_type'] ?? 'Gerät') : ($repair['device_type'] ?? 'Gerät');
...
if ($deposit > 0) {
    $pdf->SetFont('helvetica', '', 10);
    $pdf->SetX($sum_x);
    $pdf->Cell($sum_lw, 6, 'Anzahlung:', 0, 0, 'R');
    $pdf->Cell($sum_rw, 6, '- ' . number_format($deposit, 2, ',', '.') . ' €', 0, 1, 'R');
}
