...
if ($snapshot !== null) {
    ...
    if ($customer_snap) {
        ...
        $cust_name    = $quote['company_name'] ?? '';
        $cust_address = '';
        $cust_phone   = '';
        $cust_email   = '';
    }
    // Firmenname bevorzugt anzeigen, falls das Angebot einer Firma zugeordnet ist.
    ...
    $tax_rate   = (float)($quote['tax_rate'] ?? get_setting('tax_rate', '0'));
    $ustg       = function_exists('pdf_ustg_active') ? pdf_ustg_active() : ($tax_rate <= 0);
    $tax_amount = $ustg ? 0.0 : round($subtotal * $tax_rate / 100, 2);
...
foreach ($items_rows as $item) {
    ...
    $drawRow($pdf, $col_w, $pos, $row_colors, $desc, (string)$qty, $unit_price, $line_total);
}
