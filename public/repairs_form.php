...
// ── Felder / Defaults ────────────────────────────────────────────────────────
$device_types   = ['Smartphone','Tablet','Laptop','PC/Desktop','Spielekonsole','Smartwatch','Sonstiges'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $adv   = $fields['advance_payment'] !== '' ? (float)str_replace(',', '.', $fields['advance_payment']) : null;
        ...
        if ($is_edit) {
            ...
            $stmt = $db->prepare(
                'UPDATE repairs SET
                    customer_id = ?, device_type = ?, manufacturer = ?, model = ?,
                    ...
                    <div class="form-group">
                        ...
                        <select id="device_type" name="device_type" class="<?= isset($errors['device_type']) ? 'input-error' : '' ?>">
                            ...
                            <?php foreach ($device_types as $dt): ?>
                                <option value="<?= h($dt) ?>" <?= $fields['device_type'] === $dt ? 'selected' : '' ?>><?= h($dt) ?></option>
                            <?php endforeach; ?>
                        </select>
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group full">
                        ...
                        <textarea id="internal_notes" name="internal_notes"
                                  rows="3"
                                  placeholder="Nur für interne Zwecke – nicht auf Kundenbelegen sichtbar …"><?= h($fields['internal_notes']) ?></textarea>
                    </div>
                </div>
            </fieldset>
