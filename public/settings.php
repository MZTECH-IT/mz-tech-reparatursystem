...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_company') {
        ...
        // Hinweistext nur Textarea-Trim (kein trim der inneren Zeilenumbrüche nötig).
        set_setting('ustg_notice_text', trim($_POST['ustg_notice_text'] ?? ''));
    ...
    if ($action === 'save_smtp') {
        ...
        foreach ($smtp_fields as $field) {
            // Passwort nur überschreiben wenn ausgefüllt
            if ($field === 'smtp_pass' && trim($_POST[$field] ?? '') === '') continue;
            set_setting($field, trim($_POST[$field] ?? ''));
        }
        log_activity('settings_updated', 'settings', null, 'SMTP-Einstellungen aktualisiert');
        flash('success', 'SMTP-Einstellungen erfolgreich gespeichert.');
    ...
    if ($action === 'test_smtp') {
        ...
        if (!$to) {
            flash('error', 'Bitte zuerst eine "Von-E-Mail" konfigurieren.');
        } else {
            $ok = send_email(
                $to,
                get_setting('smtp_from_name', 'MZ Tech'),
                'MZ Tech – SMTP-Testmail',
                '<p>Die SMTP-Konfiguration funktioniert korrekt!</p>',
                'Die SMTP-Konfiguration funktioniert korrekt!'
            );
            if ($ok) {
                flash('success', 'Test-E-Mail erfolgreich gesendet an ' . $to);
            } else {
                flash('error', 'Fehler beim Senden. Bitte SMTP-Einstellungen prüfen (Details im Error-Log).');
            }
        }
        header('Location: ' . url('settings.php') . '#smtp');
        exit;
...
$company_keys = [
    'company_name', 'company_address', 'company_phone', 'company_email',
    'company_website', 'company_iban', 'company_bic', 'company_tax_id',
    'invoice_prefix', 'repair_prefix', 'warranty_default', 'tax_rate', 'currency_symbol',
];
$settings = [];
    ...
    <form method="post">
      ...
      <div class="form-grid">
        ...
        <div class="form-group">
          ...
          <small class="form-hint">0 = Kleinunternehmer nach § 19 UStG (keine MwSt. auf Rechnungen &amp; Kostenvoranschlägen). Bei &gt; 0 wird die MwSt. regulär ausgewiesen.</small>
        </div>
        <div class="form-group">
        ...
        <div class="form-group" style="grid-column:1/-1;">
          ...
          <textarea id="ustg_notice_text" name="ustg_notice_text" class="form-control" rows="2"><?= h($settings['ustg_notice_text']) ?></textarea>
          <small class="form-hint">Wird auf Rechnungen &amp; Kostenvoranschlägen anstelle des MwSt-Ausweises angezeigt, solange der MwSt-Satz oben auf 0 steht.</small>
        </div>
      </div>
...
<div class="card" id="smtp" style="margin-bottom:24px;">
  ...
  <div class="card-body">
    <form method="post">
      ...
      <div class="form-grid">
        <div class="form-group">
          ...
          <input type="text" id="smtp_host" name="smtp_host" class="form-control" value="<?= h($settings['smtp_host']) ?>" placeholder="smtp.example.com">
        </div>
        <div class="form-group">
          <label for="smtp_port">SMTP-Port</label>
          <input type="number" id="smtp_port" name="smtp_port" class="form-control" value="<?= h($settings['smtp_port'] ?: '587') ?>" min="1" max="65535">
        </div>
        <div class="form-group">
          <label for="smtp_user">SMTP-Benutzername</label>
          <input type="text" id="smtp_user" name="smtp_user" class="form-control" value="<?= h($settings['smtp_user']) ?>" autocomplete="username">
        </div>
        <div class="form-group">
          <label for="smtp_pass">SMTP-Passwort</label>
          <input type="password" id="smtp_pass" name="smtp_pass" class="form-control" placeholder="Leer lassen = unverändert" autocomplete="current-password">
        </div>
        <div class="form-group">
          <label for="smtp_from_name">Absendername</label>
          <input type="text" id="smtp_from_name" name="smtp_from_name" class="form-control" value="<?= h($settings['smtp_from_name']) ?>">
        </div>
        <div class="form-group">
          <label for="smtp_from_email">Absender-E-Mail</label>
          <input type="email" id="smtp_from_email" name="smtp_from_email" class="form-control" value="<?= h($settings['smtp_from_email']) ?>">
        </div>
      </div>
      <div style="margin-top:16px;display:flex;gap:12px;flex-wrap:wrap;">
        <button type="submit" class="btn btn-primary"><?= svg_icon('check') ?> SMTP speichern</button>
      </div>
    </form>
    <!-- Test-Mail separat -->
    <form method="post" style="margin-top:12px;display:inline;">
      ...
      <button type="submit" class="btn btn-outline"><?= svg_icon('mail') ?> Test-E-Mail senden</button>
    </form>
  </div>
</div>
